...
def plot(options):
    ...
    velocityRange = np.array([[-7,-7],[7,7]])
    ...
    scatterVectorScale = 0.5
    gridVectorScale = 0.5

    quiverOpts = {'headwidth':4, 'headlength':2}
    ...
    print("Final mask:", imageFinalMask.shape, imageFinalMask.dtype)
    ...
    if residualsFound: 
      ...
      # mask can result in to a single bool
      if isinstance(mask,np.bool_):
        mask = np.ones(imageFinalMask.shape,dtype=bool)*mask
